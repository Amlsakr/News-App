...
class NewsRepositoryImplementation @Inject constructor(
    ...
    override suspend fun getTopHeadLinesByCategory(category: String): Resource<NewsResponse> {
        return try {
            Resource.Success(data = newsApi.getTopHeadlinesByCategory(category = category))
        } catch (e: Exception) {
            e.printStackTrace()
